...
fn main() {
    let supra_seal_path =
        PathBuf::from(env::var("SUPRA_SEAL_PATH").expect("SUPRA_SEAL_PATH must be set"));

    let cpp_lib_dir = supra_seal_path
        .join("obj")
        .canonicalize()
        .expect("cannot canonicalize path");

    let spdk_lib_dir_buf = supra_seal_path
        .join("deps/spdk-v22.09/build/lib")
        .canonicalize()
        .expect("cannot canonicalize path");
    let spdk_lib_dir = spdk_lib_dir_buf.to_str().unwrap();

    let dpdk_lib_dir_buf = supra_seal_path
        .join("deps/spdk-v22.09/dpdk/build/lib")
        .canonicalize()
        .expect("cannot canonicalize path");
    let dpdk_lib_dir = dpdk_lib_dir_buf.to_str().unwrap();

    let dpdk_env_path = supra_seal_path
        .join("deps/spdk-v22.09/build/lib/libspdk_env_dpdk.a")
        .canonicalize()
        .expect("cannot canonicalize path");
